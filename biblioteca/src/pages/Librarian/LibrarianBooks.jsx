...
export default function LibrarianBooks() {
  ...
  // Novos estados para gerenciamento de exclusão local
  ...
  // Monitoramento do teclado para exclusão em memória
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (e.key === "Delete" || e.key === "Backspace") {
        if (selectedRowIndex === dados.length - 1) return; // Ignora a linha padrão vazia
  ...
  const salvarAlteracoes = async () => {
    ...
    setLoading(true);
    try {
      // 1. Efetiva as remoções pendentes no banco de dados
      ...
      // 2. Realiza o Upsert das linhas restantes ou novas

...
// Papéis disponíveis no cadastro (admin não se cadastra pelo formulário)
const PAPEIS = [
  { valor: "estudante",    label: "Sou Estudante" },
  { valor: "bibliotecario", label: "Sou Bibliotecário" },
];
...
export default function Login() {
  ...
  return (
    <div
      className="min-h-screen flex items-center justify-center bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-800 via-slate-950 to-black font-sans p-6"
      style={{
        ...
        {success ? (
          ...
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="flex gap-3 p-1 bg-black/20 rounded-2xl border border-white/5">
                    {PAPEIS.map(({ valor, label }) => (
                      <button
                        key={valor}
                        type="button"
                        className={`flex-1 py-2.5 rounded-xl text-[10px] uppercase tracking-wider font-black transition-all ${formData.papel === valor ? "bg-white/10 text-blue-400 border border-blue-500/20" : "text-slate-600"}`}
                        onClick={() => setFormData({ ...formData, papel: valor })}
                      >
                        {label}
                      </button>
                    ))}
              ...
              {error && (
                <p className="text-[11px] font-bold text-red-400 text-center uppercase tracking-wide">
                  {error}
                </p>
              )}
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full mt-4 py-4 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-2xl shadow-lg shadow-blue-900/20 transition-all active:scale-95 disabled:opacity-50"
              >
